...
def lambda_handler(event, context):
    try:
        ...
        # Build ranked list for professors that had embeddings (already filtered).
        ranked_professors = []
        for idx, similarity in zip(ranking_indices, sorted_similarities):
            prof = prof_entries[idx].copy()
            prof['score'] = float(similarity)
            ranked_professors.append(prof)

        # Collect professors without embeddings so they can be appended at the end.
        excluded_professors = []
        for entry in profs:
            if not entry.get('tag_embeddings'):
                prof = entry.copy()
                prof['score'] = None
                excluded_professors.append(prof)
